...
iptables -t nat -A PREROUTING -p tcp -i $IFWAN -d $IPWAN --dport 80 -j DNAT --to-destination $SECONDTCONTAINERIP:80
iptables -A FORWARD -p tcp -i $IFWAN -d $SECONDTCONTAINERIP --dport 80 -o $IFINT -j ACCEPT
iptables -A FORWARD -p tcp -o $IFWAN -s $SECONDTCONTAINERIP --sport 80 -i $IFINT -m state --state RELATED,ESTABLISHED -j ACCEPT
iptables -t nat -A POSTROUTING -p tcp -o $IFWAN -s $SECONDTCONTAINERIP --sport 80 -j SNAT --to-source $IPWAN:80
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
